...
public protocol SerializableNode: Loggable {
    func writeTo(db: Connection, with foreignKeys: any SerializableForeignKeys) throws
...
extension SerializableNode {
    public func writeIfNotExists(db: Connection, with foreignKeys: any SerializableForeignKeys, propagate: Bool) throws {
        if !(try self.existsOn(db: db, with: foreignKeys, propagate: propagate)) {
            print("\(#function) writing model to db")
            try self.writeTo(db: db, with: foreignKeys)
        }
    }
}
